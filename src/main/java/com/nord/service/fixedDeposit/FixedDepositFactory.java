...
public class FixedDepositFactory implements IFixedDepositFactory {
  ...
  public IGetFixedDeposit getFdByPlan(String selectedTerm) {
    return new GetFixedDepositByPlan().setView(createPlanView()).setSelectedFd(createUserFdModel()).setSelectedTerm(selectedTerm);
  }

  @Override
  public IGetFixedDeposit getFdByEstimation() {
    return new GetFixedDepositByEstimation().setSelectedFd(createUserFdModel()).setView(createPlanView());
  }

  @Override
  public IFixedDepositConfirmation createFdConfirmation(IUserFixedDepositModel selectedFd, List<IFixedDepositPlansModel> fixedDepositPlans) {
    return new FixedDepositConfirmation().setView(createFdConfirmationView()).setUserPersistence(createUserFdDb()).setFixedDepositPlans(fixedDepositPlans).setSelectedFd(selectedFd).setUpgradeableFd(createUserFdModel());
  }

  @Override
  public IGetUserFixedDeposits createGetUserFd(List<IFixedDepositPlansModel> fixedDepositPlans) {
    return new GetUserFixedDeposits().setDb(createUserFdDb()).setFdPlans(fixedDepositPlans).setView(createUserPlansView());
  }

  @Override
  public ICreateFixedDeposit createFixedDeposit() {
    return new CreateFixedDeposit().setFactory(this);
  }
