...
public class CreditScoreLoanService implements ICreditScoreLoanService {
  ...
  public void creditScoreLoanHomePage() {
    while (true) {
      ...
      switch (input) {
        case 1:
          factory.creditScoreLoanHistory().setView(view).setFactory(factory).showLoanHistory(userLoans);
          break;
        case 2:
          factory.applyForCreditLoan().setView(view).setFactory(factory).applyForAll();
          updateCreditLoanService();
          ;
          break;
        case 3:
          factory.payForCreditLoan().setView(view).setFactory(factory).payCreditLoanByGivenLoans(userLoans);
          updateCreditLoanService();
          break;
        case 4:
  ...
  public boolean updateCreditLoanService() {
    this.userLoans = this.factory.getCreditScoreLoanHistoryModel().getCreditScoreLoanHistory();
    return true;
  }
}
