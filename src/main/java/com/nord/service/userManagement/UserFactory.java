...
public class UserFactory implements IUserFactory {
  ...
  public IUserService createCustomer() {
    return new CustomerService().setFactory(createMainFactory()).setNotification(createNotificationDb()).setView(createCustomerMenuView()).setDbObj(createUserAuthDb());
  }

  @Override
  public IUserService createAdmin() {
    return new AdminService().setFactory(this).setView(createAdminMenuView()).setDbObj(createUserAuthDb());
  }
  ...
  public INewUser createNewCustomer() {
    return new NewUser().setAdmin(false).setPersistence(createDetailsModel()).setView(createNewUserView());
  }

  @Override
  public INewUser createNewEmployee() {
    return new NewUser().setAdmin(true).setPersistence(createDetailsModel()).setView(createNewUserView());
  }
  ...
  public IFeedbackServiceAdminSide createFeedbackAdminService() {
    return new FeedbackServiceAdminSide().setFactory(createFeedbackFactory()).setView(createFeedbackAdminSideView());
  }
  ...
  public IBranchLocator createBranchLocator() {
    return new BranchLocatorService().setFactory(this).setDb(createBranchLocatorModel()).setView(createBranchLocatorView());
  }

  @Override
  public ILoanApproval createLoanApproval() {
