...
public class CreditCardRecommendationService implements ICreditCardRecommendationService {
  ...
  public void creditCardRecommendationHomepage() {
    while (true) {
      ...
      switch (input) {
        case 1:
          factory.payCreditCardBill().setView(view).setFactory(factory).payCreditCardBill();
          break;
        case 2:
          factory.upgradeCreditCard().setFactory(factory).setView(view).assignCreditCardToUser();
          break;
        case 3:
          factory.applyForCreditCard().setFactory(factory).setView(view).assignCreditCardToUser();
          break;
        case 4:
          factory.recommendCreditCard().setView(view).setCreditCardTypeView(typesView).recommendCreditCard();
          break;
        case 9:
          return;
        default:
