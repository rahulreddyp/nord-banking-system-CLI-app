...
public class UserFixedDepositsPersistence extends AbstractRewardPoints implements IUserFixedDepositsPersistence {
  ...
  public boolean addFdToUser(IUserFixedDepositModel fd) {
    try {
      ...
      String query = "INSERT INTO customer_fixed_deposit (fd_id ,user_id, start_date,withdrawal_date," + "initial_amount,withdrawal_amount, plan_id)" + " VALUES (?, ?, ?, ?, ?, ?, ?)";
      ...
      addNotification(Constants.FIXED_DEPOSIT_NOTIF, fd.getWithdrawalDate().minusDays(15));
  ...
  public List<IUserFixedDepositModel> getAllFdsForUser() {
    ...
    try {
      ...
      ResultSet resultSet = statement.executeQuery("SELECT * FROM customer_fixed_deposit WHERE user_id='" + Context.getLoggedInUserId() + "';");
  ...
  public boolean withdrawFd(int fdId, double amount) {
    ...
    try {
      Connection dbConnection = DbConnection.getConnection();
      Statement statement = dbConnection.createStatement();
      statement.executeUpdate("DELETE FROM customer_fixed_deposit " + "WHERE fd_id='" + fdId + "';");
