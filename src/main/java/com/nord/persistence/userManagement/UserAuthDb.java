...
public class UserAuthDb extends AbstractUserDb implements IUserAuthDb {
  ...
  public String getHashPassword(String userName, boolean isAdmin) {
    String result = "";
    String query = "SELECT password FROM customer_details WHERE email='" + userName + "'" + " AND is_admin = " + isAdmin + " AND is_active = 1;";
    Statement statement;
    try {
  ...
  public boolean setPassword(String password, String username) {
    String query = "UPDATE customer_details SET password ='" + Utils.encryptToSha256(password) + "', is_new_customer = 0" + " WHERE email='" + username + "';";
    Statement statement;
    try {
      Connection dbConnection = DbConnection.getConnection();
      statement = dbConnection.createStatement();
