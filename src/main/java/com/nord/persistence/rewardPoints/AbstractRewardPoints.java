...
public abstract class AbstractRewardPoints extends AbstractBanking {
  ...
  public boolean addRewardPoints(int pointsToAdd) {
    ...
    String query = "INSERT INTO reward_points(reward_point_add_date, " + "reward_point_expire_date, reward_point_balance, user_id) " + "VALUES (?, ?, ?, ?)";
    try {
  ...
  public void removeExpiredPoints() {
    String query = "DELETE FROM reward_points WHERE " + "reward_point_expire_date <= CURDATE() AND user_id = " + Context.getLoggedInUserId() + ";";
    try {
      Connection dbConnection = DbConnection.getConnection();
