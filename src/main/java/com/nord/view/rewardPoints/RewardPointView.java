...
public class RewardPointView extends AbstractUserInterface implements IRewardPointView {
  ...
  public void showRewardPoints(List<IRewardPointModel> list) {
    showHeader();
    System.out.println(String.format("%10s %10s %20s %20s", "Points", "Dollars", "Add Date", "Expiry Date"));
    for (IRewardPointModel item : list) {
      System.out.println(String.format("%10s %10s %20s %20s", item.getRewardPointBalance(), item.getDollarAmount(), item.getRewardPointAddDate(), item.getRewardPointExpireDate()));
    }
  ...
  public void successfulConversion(double dollarAmount) {
    showHeader();
    System.out.println("Congrats! $" + dollarAmount + " have been added to your " + "bank balance");
    System.out.println("0. Back to previous menu");
  }
}
